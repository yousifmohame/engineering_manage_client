...
export default function PartnershipExpensesPage() {
  ...
  const initialForm = {
    ...
    categories: ["مكتب"], // أصبح مصفوفة لدعم التحديد المتعدد
    amount: "",
    currency: "SAR",
    description: "",
    isRecurring: false,
    recurrenceRate: "شهري", // الافتراضي
    attachmentFile: null,
    attachmentUrl: null,
  };
  ...
  const filteredExpenses = showRecurringOnly
  ...
  const toggleCategory = (cat) => {
    setForm((prev) => {
      if (prev.categories.includes(cat)) {
        return {
          ...prev,
          categories: prev.categories.filter((c) => c !== cat) || ["أخرى"],
        };
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // إعداد البيانات كـ FormData لرفع الملفات
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
          ...
          >
            ...
            <div className="relative z-10 space-y-1.5">
              ...
              <p className="text-[11px] sm:text-xs font-black drop-shadow-sm">
                توقع مصاريفك المستقبلية
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-50 text-[11px]">
                {displayExpenses.map((exp) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-black text-slate-800 max-w-[150px] truncate">
                      {exp.description}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {exp.category?.split(",").map((cat, i) => (
                          <span
                            key={i}
                            className={`inline-flex px-2 py-0.5 rounded text-[9px] font-black border bg-slate-100 text-slate-700 border-slate-200`}
                          >
                            {cat}
                          </span>
                        ))}
                      ...
                      {exp.isRecurring && (
                        <span className="mt-1 inline-block px-1.5 py-0.5 bg-amber-50 text-amber-700 text-[9px] font-bold rounded border border-amber-200">
                          <Calendar size={9} className="ml-0.5 inline" />{" "}
                          {exp.recurrenceRate || "متكرر"}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {/* تمت إزالة opacity-0 لتكون الأزرار ظاهرة دائماً */}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase">
                      التصنيف (يمكنك اختيار أكثر من واحد)
                    </label>
                  ...
                  <div>
                    <InputCompact
                      ...
                      placeholder="مثال: دفعة مقاولات، شراء مكتب..."
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-500 uppercase flex items-center gap-1">
                        <Paperclip size={12} /> المرفقات (إيصال/فاتورة)
                      </label>
                      <input
                        type="file"
                        accept="image/*,.pdf"
    ...
    <div className="p-3 space-y-2.5">
      <div className="flex justify-between items-start gap-2">
        <div className="min-w-0 flex-1">
          ...
          <p className="font-black text-slate-800 text-sm mt-0.5 truncate">
            {item.description}
          </p>
        </div>
      ...
      <div className="flex items-center justify-between pt-1.5 border-t border-slate-100">
        ...
        <span className="font-black text-red-700 text-base">
          ...
          <span className="text-[10px] font-normal opacity-70">SAR</span>
        </span>
      </div>
    </div>
  );
...
// مكونات SummaryCard, InputCompact, CurrencyDisplay تظل كما هي في كودك المرفق
function SummaryCard({ title, amount, color, icon }) {
  return (
    <div
      className={`bg-gradient-to-br ${color} text-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-lg relative overflow-hidden active:scale-[0.98] transition-transform`}
    >
      <div className="absolute -right-3 -top-3 w-16 h-16 bg-white opacity-10 rounded-full blur-xl"></div>
      <div className="relative z-10 space-y-1.5">
        <h3 className="text-[10px] font-bold uppercase tracking-wide flex items-center gap-1 opacity-90">
        ...
        <p className="text-lg sm:text-xl font-black drop-shadow-sm">
          ...
          <span className="text-[10px] font-normal opacity-75">SAR</span>
        </p>
